...
class PartialDependenceTabular(TabularExplainer):
    ...
    def __init__(self, training_data: Tabular, predict_function, mode="classification", **kwargs):
        ...
        for column_index, column_name in enumerate(self.feature_columns):
            arr = self.data.iloc[:, column_index][0]
            num_unique_values = len(np.unique(arr))
            if column_index in self.categorical_features or num_unique_values <= grid_resolution:
                # Categorical features
                candidates = sorted(np.unique(arr))
            else:
                # Continuous-valued features
                percentiles = np.linspace(0, 100, num=grid_resolution)
                
                if isinstance(arr[0], np.complex64):
                    candidates = sorted(set(np.percentile(arr.real, percentiles)))
                else:
                    candidates = sorted(set(np.percentile(arr, percentiles)))
    ...
    def _compute_pdp(self, data, column_index):
        ...
        for i, y in enumerate(candidates):
            if isinstance(x.iloc[0,0], list):
                y_shape = len(x.iloc[0,0])
            else:
                y_shape = x.iloc[0,0].shape[0]
            big_y = [y] * y_shape
            #x.iloc[:, column_index] = np.array([big_y] * len(x.iloc[:, column_index]))
            x[x.columns[column_index]] = [big_y] * len(x.iloc[:, column_index])
            y_hat = self.predict_fn(x)
            baselines.append(y_hat)
