...
class PDPExplanation(ExplanationBase):
    ...
    def plot(self, class_names=None, xlabel=None, ylabel="Partial dependence plots", feature_names=None, dB=False, already_log=False,**kwargs):
        """
        ...
        for i, feature in enumerate(features):
            feature_name = feature if feature_names is None else feature_names[i]
            ...
            if dB:
                # coming from a magnitude spectrogram: dB = 20 * log10(x)
                if not already_log:
                    values = [20*np.log10(float(v)) for v in values]
                # coming from a log power spectrogram: dB = 10 * x
                else:
                    values = [10 * float(v) for v in values]
